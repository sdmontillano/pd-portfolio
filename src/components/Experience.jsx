import { motion } from 'framer-motion'
import { experience } from '../data/portfolioData'

const stagger = {
  initial: { opacity: 0 },
  whileInView: { opacity: 1, transition: { staggerChildren: 0.15 } },
}

const slideIn = {
  initial: { opacity: 0, x: -20 },
  whileInView: { opacity: 1, x: 0, transition: { duration: 0.5, ease: 'easeOut' } },
}

export default function Experience() {
  return (
    <section id="experience" style={{ background: 'var(--surface)', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
      <div className="container">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: '-50px' }}
          transition={{ duration: 0.5 }}
        >
          <div className="section-label">Experience</div>
          <div className="vermillion-bar" />
          <div className="section-title">
            Work <span>&amp; Internship</span>
          </div>
        </motion.div>

        <motion.div variants={stagger} initial="initial" whileInView="whileInView" viewport={{ once: true, margin: '-50px' }}>
          {experience.map((exp, i) => (
            <motion.div key={i} className="exp-grid" style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 32, marginBottom: 24 }} variants={slideIn}>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--red)' }}>{exp.period}</div>
                <div style={{ fontSize: 12, color: 'var(--text-light)', marginTop: 4 }}>{exp.duration}</div>
              </div>
              <div style={{ position: 'relative', paddingLeft: 24, borderLeft: '2px solid var(--border)' }}>
                <motion.div
                  style={{
                    position: 'absolute',
                    left: -5,
                    top: 6,
                    width: 8,
                    height: 8,
                    background: 'var(--red)',
                    borderRadius: '50%',
                  }}
                  initial={{ scale: 0 }}
                  whileInView={{ scale: 1 }}
                  viewport={{ once: true }}
                  transition={{ type: 'spring', stiffness: 200, damping: 15, delay: 0.2 }}
                />
                <div style={{ fontSize: 17, fontWeight: 600 }}>{exp.role}</div>
                <div style={{ fontSize: 14, color: 'var(--text-light)', marginTop: 2 }}>{exp.company}</div>
                <div style={{ marginTop: 10 }}>
                  {exp.highlights.map((h, j) => (
                    <div key={j} style={{
                      fontSize: 14,
                      color: 'var(--text-light)',
                      paddingLeft: 16,
                      position: 'relative',
                      marginBottom: 4,
                    }}>
                      <span style={{ position: 'absolute', left: 0, color: 'var(--red)' }}>&mdash;</span>
                      {h}
                    </div>
                  ))}
                </div>
              </div>
            </motion.div>
          ))}
        </motion.div>
      </div>
    </section>
  )
}
